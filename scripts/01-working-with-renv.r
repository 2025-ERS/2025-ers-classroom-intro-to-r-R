...
# A major strength, but also potential challenge of R is that the software changes all the time. All functionality # of R comes from packages, and these are improved, expanded, updated all the time. Say that you develop a set of 
# scripts in R now, for example to analyse the data in your master project., and you develop a publication from the # results. But you get several revisions, and finally complete the paper two years later. By that time, you will 
# # have updated your versions of R and your packages several times.This may change the functionality of your 
# scripts, they may not work anymore. And when you publish the paper and deposit the data and scripts in a
# repository, even 10 years from now you want to be able to replicate your analyses, with the "old" versions of the
...
# package installed in their R library. This means only one person can improve the scripts and run them, instead of
# everyone contributing.

# The renv package solves this in combination with using an R Studio project and Git/Github. You use it to create
# an renv.lock file with renv::init(). You typically only do this once. This initialization of your project 
# serves two goals:
...
# The above section you always want to include at the start of your script!